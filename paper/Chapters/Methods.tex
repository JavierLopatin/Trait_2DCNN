...
\subsection{PROSAIL Radiative Transfer Simulations}
\label{sec:prosail}

To investigate whether models can be trained entirely on physically simulated spectra and applied to real observations---thereby eliminating the need for labeled field data---we generated a synthetic training dataset using the PROSAIL radiative transfer model. PROSAIL couples the leaf optical properties model PROSPECT-PRO \citep{feret2021prospectpro} with the canopy bidirectional reflectance model 4SAIL \citep{verhoef2007sail}, simulating top-of-canopy spectral reflectance from biophysical input parameters that correspond directly to the target traits.

A look-up table (LUT) of 50,000 simulations was generated using the \texttt{prosail} R package \citep{feret2024prosailR}. Input parameter distributions followed the Algorithm Theoretical Basis Document (ATBD) of the Sentinel-2 biophysical processor \citep{weiss2020atbd}, which specifies truncated Gaussian distributions for leaf-level parameters (C\textsubscript{ab}: 20--90\,$\mu$g\,cm\textsuperscript{$-$2}, mode = 45; C\textsubscript{m}: 0.003--0.011\,g\,cm\textsuperscript{$-$2}, mode = 0.005) and canopy-level parameters (LAI: 0--15, mode = 2, $\sigma$ = 3). PROSPECT-PRO-specific parameters not covered by the ATBD---carotenoids (C\textsubscript{ar}), anthocyanins (C\textsubscript{anth}), and protein content (C\textsubscript{p})---were assigned distributions derived from the labeled GreenHyperSpectra data. The constraint C\textsubscript{p} + C\textsubscript{bc} = C\textsubscript{m} was enforced, and co-distributions between parameters and LAI were applied following Table~6 of the ATBD, such that parameter ranges narrow for high LAI values (e.g., canopies with high leaf area index are unlikely to have very low chlorophyll content). Fixed parameters followed \citet{cherif2025greenhyperspectra}: brown pigment content = 0.25, structural coefficient $N_s$ = 1.5, leaf inclination distribution (LIDF, Beta index) = 5, soil brightness fraction = 0.8, hotspot parameter = 0.01, and observation geometry of $\theta_s$ = 30\textdegree, $\theta_v$ = 0\textdegree, $\psi$ = 0\textdegree.

The simulated spectra underwent post-processing to match the characteristics of real observations. Reflectance values were clipped (negative values set to zero; values exceeding unity linearly interpolated from neighbors), and Savitzky-Golay smoothing (window = 65, polynomial order = 1) was applied per spectral segment, replicating the preprocessing of GreenHyperSpectra. Water absorption bands were removed at the same wavelength intervals, yielding 1721 bands. Finally, multiplicative Gaussian noise ($\mu = 0$, $\sigma = 0.02$) was added to each spectral value to simulate measurement uncertainty and prevent overfitting on noiseless synthetic data \citep{ferreira2025enhancing}. The resulting 50,000 labeled synthetic spectra serve as training data for experiments evaluating synthetic-to-real domain transfer (Section~\ref{sec:results_prosail}).
...
\textit{Case Study 2: Self-supervised pretraining and domain transfer.} Three experiments assessed the impact of pretraining strategies, all using Reshape as the fixed transformation: (i) supervised training without pretraining (the Case Study 1 baseline); (ii) MAE-2D pretraining on 139K unlabeled images followed by fine-tuning on real labeled data, compared against the MAE-1D of \citet{cherif2025greenhyperspectra} ($R^2 = 0.645$); and (iii) MAE-2D pretraining followed by fine-tuning on 50K synthetic PROSAIL spectra and evaluation on real test data, representing a zero-real-labels scenario in which no field-measured trait data are used for model calibration.

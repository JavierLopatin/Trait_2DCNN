...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/transforms_example.pdf}
    \caption{Illustration of the six 1D-to-2D spectral transformations applied to three representative spectra with contrasting trait profiles. (a) Original reflectance spectra: Spectrum~1 exhibits high chlorophyll content and high LAI (strong visible absorption and near-infrared plateau), Spectrum~2 shows low chlorophyll and high leaf mass per area (reduced visible absorption, prominent dry matter features), and Spectrum~3 displays high anthocyanin content (elevated green-edge reflectance). (b) Corresponding 2D image representations produced by each transformation method. Each transformation encodes different aspects of the spectral information: Reshape preserves wavelength adjacency as spatial texture; CWT reveals multi-scale absorption features; Spectrogram captures localized frequency content through three complementary windows; 2D-COS highlights inter-band correlations; GAF encodes angular relationships between spectral values; and MTF represents transition dynamics between discretized intensity bins.}
    \label{fig:transforms_example}
\end{figure}
...
\sm{ACA AGREGARIA ALGO DICIENDO QUE DADO QUE TENEMOS VALORES EN DOS DIMENSIONES, ENTONCES AHORA VAMOS A HACER ESTO. TODAVIA NOSE LAS CLASES DE LOS VALORES}

\jl{Having converted each spectrum into a 2D image, we now apply image models to predict the traits. We treated the task as a single multi-output regression, where one model predicts all eight traits (Section~\ref{sec:data}) at once instead of training eight separate models. We tested two settings. We first trained a supervised CNN from scratch, and then pretrained a self-supervised MAE on unlabeled images before fine-tuning it on the labeled data.}

\subsubsection{Supervised Regression: EfficientNet-B0}
\label{sec:efficientnet}

For supervised multi-trait regression, we employed EfficientNet-B0 \citep{tan2019efficientnet} as the backbone architecture, instantiated through the timm library \citep{rw2019timm} and trained from scratch. EfficientNet-B0 was selected for its favorable trade-off between model capacity ($\sim$4\,M trainable parameters) and computational efficiency, which proved critical given the moderate size of the labeled training set ($n = 4{,}508$). It is also the selected model for the 1D baseline in \citet{cherif2025greenhyperspectra}. \sm{NO CACHE, TIENES 8 REGRESIONES DISTINTAS?}
 %Training from scratch also ensures a fair comparison with the 1D baseline of \citet{cherif2025greenhyperspectra}, which likewise does not use pretrained weights. 
 The network's final classification layer was replaced with \jl{a single} linear head mapping to 8 output neurons (one per target trait)\jl{, so one network predicts all traits jointly}, and a dropout rate of 0.3 was applied before the output layer. For single-channel transformations (Reshape, CWT, MTF), the first convolutional layer was adapted from three to one input channel by averaging the randomly initialized channel weights along the input dimension. Multi-channel transformations (Spectrogram: 3 channels; 2D-COS and GAF: 2 channels) required analogous channel-count adjustments.
...
\begin{equation}
    ...
    \label{eq:mae_loss}
\end{equation}
where $\mathcal{M}$ denotes the set of masked patch indices, $\hat{\mathbf{p}}_i$ is the reconstructed patch, and $\mathbf{p}_i$ is the original patch. No data augmentation beyond random masking was applied during pretraining \sm{A QUE TE REFIERES? ESCONDER PARTE DE LA MASCARA? se pierde la idea, pero se entiende despues cuando VOLVEMOS A HABLAR DE 8 CLASES}\jl{. Here masking means hiding random image patches, which is not the same as the missing-label mask used in the loss of Equation~\ref{eq:masked_mse}}, consistent with the finding of \citet{he2022mae} that masking itself provides sufficient regularization, and following the protocol of \citet{cherif2025greenhyperspectra} whose 1D MAE likewise omitted augmentation.
...
The supervised EfficientNet-B0 models were optimized with AdamW \citep{loshchilov2019adamw} ($\beta_1 = 0.9$, $\beta_2 = 0.999$, weight decay = $10^{-4}$) and a cosine annealing learning rate schedule with initial rate $10^{-3}$ and minimum rate $10^{-6}$. Training proceeded for a maximum of 100 epochs with early stopping (patience = 15 epochs, monitoring validation loss). Within the training set, 15\% of samples were withheld for validation using random splitting. The batch size was set to 16.
...
\begin{equation}
    ...
    \label{eq:masked_mse}
\end{equation}
where $\hat{y}_{ij}$ and $y_{ij}$ are the predicted and observed values for sample $i$ and trait $j$, and $m_{ij}$ is a binary mask indicating whether the observation is available. During training, two forms of data augmentation were applied to the training set: (i) an additive baseline shift drawn uniformly from $\pm$2\% of the spectral range, and (ii) a multiplicative scaling factor drawn uniformly from $[0.98, 1.02]$, both applied to the raw spectra before transformation \sm{COMO ESTAS TRANSFORMACIONES TE CAMBIAN LA RELACION DEL INPUT X CON EL OUTPUT Y, ES DECIR, PODRIA PARSAR QUE AL AHCER ESTE CAMBIO LA ETIQUETA DADA YA NO SEA CORRECTA?}\jl{. These perturbations are small and label-preserving, because they emulate illumination and sensor variation that leaves the underlying trait unchanged, so the trait label stays valid. \citet{cherif2023spectra} used the same augmentation scheme}. For pre-computed (cached) images, small Gaussian noise ($\sigma = 0.01$) was added to the transformed images at each epoch. Prior to training, trait values were transformed using the Yeo-Johnson power transformation \citep{yeo2000power} fitted on the training set, which stabilizes variance and reduces the influence of skewed distributions. Predictions were inverse-transformed for evaluation in original units.

\subsubsection{MAE Pretraining}

The MAE-2D was pretrained on Reshape images derived from the 139,295 unlabeled spectra (after filtering 375 samples with missing data, yielding 138,920 valid images). Training used AdamW ($lr = 10^{-4}$, weight decay = $10^{-4}$) with cosine annealing over 300 epochs, a batch size of 32, and gradient clipping (max norm = 1.0). A 95/5 train/validation split was used for monitoring reconstruction loss, with early stopping at patience = 30 epochs. The pretraining objective was pure MSE reconstruction of masked patches (Equation~\ref{eq:mae_loss}), without additional augmentation, as random masking already provides effective regularization \citep{he2022mae}.

\subsubsection{Fine-Tuning}

After MAE pretraining, the decoder was discarded and the encoder was equipped with a regression MLP head. Full fine-tuning was performed, updating all encoder parameters jointly with the regression head ($lr = 10^{-4}$). Fine-tuning used the same protocol as supervised training (AdamW, early stopping, masked MSE loss) with 100 maximum epochs. \sm{y las 8 CAPAS?} \jl{The head is the same 192 $\rightarrow$ 192 $\rightarrow$ 8 MLP, whose eight outputs are the eight traits.}
...
To formally compare the performance of different transformations, we conducted pairwise paired $t$-tests on per-trait $R^2$ values across seeds ($n = 24$ observations per pair: 8 traits $\times$ 3 seeds), applying Bonferroni correction for multiple comparisons ($\alpha = 0.05 / 15 = 0.003$ for 15 pairwise tests among six transforms). This approach was further validated on the Cherif et al.\ (2023) dataset, where higher statistical power was available through 5-fold cross-validation across 20 traits ($n = 100$ per pair, Bonferroni-corrected $\alpha = 0.005$ for 10 pairs).


\subsection{\jl{Spectral Importance}}
\label{sec:importance}

\jl{We used an attribution approach to obtain the variable importance of the predicted traits. We specifically applied two standard interpretability methods to the best supervised model (Reshape + EfficientNet-B0), Integrated Gradients (IG) \citep{sundararajan2017ig} and Grad-CAM \citep{selvaraju2017gradcam}. IG produces signed, input-level attributions, which we computed using the training-set mean image as the baseline and a 50-step Riemann sum. We attributed on the normalized image that the network receives rather than on the raw spectrum. The Reshape transform applies a per-image min-max scaling, so differentiating through that step piles the gradient onto the few bands that set the scale. Attributing on the network input avoids this trait-agnostic artifact and preserves the IG completeness axiom. Grad-CAM produces region-level relevance over the final convolutional embedding.}

\jl{The Reshape transform preserves band order. Pixel $(i,j)$ maps to band $k = 42i + j$. We exploited this to unfold every 2D attribution back to the 1721-band wavelength axis. The 2D attribution maps therefore become 1D importance profiles directly comparable to classical band selection in remote sensing. Figure~\ref{fig:pipeline} illustrates this pipeline.}

\jl{We then measured how much the model concentrates importance in physically meaningful regions. For each trait we defined known absorption regions from the literature \citep{serbin2020scaling, angel2022remote}. We computed an enrichment score. It is the share of total IG importance falling inside those regions divided by the fraction of the spectrum they cover. A score above one means the model concentrates importance in known absorption regions more than expected by chance. We averaged IG profiles and enrichment scores across the three seeds.}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/fig_pipeline.pdf}
    \caption{\jl{Spectral importance pipeline. A 1D spectrum is folded into a 2D Reshape image and fed to the EfficientNet-B0 model. Grad-CAM and Integrated Gradients attribution maps are computed on the image, then unfolded back to the 1721-band wavelength axis using the band-order-preserving property of the Reshape transform.}}
    \label{fig:pipeline}
\end{figure}
